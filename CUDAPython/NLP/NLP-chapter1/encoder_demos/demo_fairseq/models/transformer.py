...
from encoder_demos.demo_fairseq import options
from encoder_demos.demo_fairseq import utils

from encoder_demos.demo_fairseq.modules import (
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, return_early=None):
        # embed tokens and positions
        x = self.embed_scale * self.embed_tokens(src_tokens)
        if return_early == "embed_scale":
            return x
        ...
        for layer in self.layers:
            x = layer(x, encoder_padding_mask, return_early=return_early)
            if return_early == 'self_attn':
                return x
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, return_early=None):

        residual = x

        x = self.maybe_layer_norm(0, x, before=True)
        x, weights = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
        if return_early == "self_attn":
            return x, weights
