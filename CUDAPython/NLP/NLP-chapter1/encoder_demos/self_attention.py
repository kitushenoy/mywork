...
from encoder_demos.demo_fairseq import data, options, tasks, tokenizer, utils
from encoder_demos.demo_fairseq.sequence_generator import SequenceGenerator
...
def demo(input_text, return_early=None):

    # return_early = "embed_scale"
    ...
    def process_batch(batch, return_early=None):
        tokens = batch.tokens
    ...
    for inputs in buffered_read([input_text], args.buffer_size):
        ...
        for batch, batch_indices in make_batches(inputs, args, src_dict, models[0].max_positions(), bpe):
            ...
            vals = process_batch(batch, return_early=return_early)
            if return_early in ['embed_scale', 'self_attn']:
                return vals
            results += vals
