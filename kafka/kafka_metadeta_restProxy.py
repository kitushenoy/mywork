...
def get_partitions(topic_name):
    ...
    res=requests.get(f"{REST_PROXY_URL}/topics/{topic_name}/partitions")
    print("Fetch paritions from kafka topic")
    try:
        res.raise_for_status()
    except:
        print("Failed to get partition details {json.dumps(resp.json(), indent=2)})")
    print(json.dumps(res.json(), indent=2))

if __name__ == "__main__":
    topics = get_topics()
    print(f"topic name :{topics[0]}")
    get_topic(topics[0])
    get_brokers()
